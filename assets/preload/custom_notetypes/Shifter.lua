...
function onUpdate(elapsed)
    for i = 0, getProperty('notes.length') - 1 do
        if getPropertyFromGroup('notes', i, 'noteType') == 'Shifter' then
            setPropertyFromGroup('notes', i, 'y', getPropertyFromGroup('notes', i, 'y') * 1.5);
        end
    end
 end
   ...
   for a = 0, getProperty('notes.length') - 1 do
      ...
      if getPropertyFromGroup('notes', a, 'noteType') == noteName then
         if (strumTime - getSongPosition()) < 1300 / scrollSpeed + 100 then
               setPropertyFromGroup("notes", a, "offsetX", lerp(getPropertyFromGroup("notes", a, "offsetX"), 0 + (sus and fixOffset or 0), elapsed * (5 * scrollSpeed)))
         end
